...
module.exports = {
  ...
  run: async (client, message) => {
    ...
    let datab = client.config.nonPrefixedUsers;
    ...
    if (command.userPrams && !message.member.permissions.has(command.userPrams)) {
      embed.setDescription(
        `\`❌\` | You need \`${command.userPrams.join(', ')}\` permission use this command.`,
      );
      return message.channel.send({ embeds: [embed] });
    }
    if (command.botPrams && !message.guild.me.permissions.has(command.botPrams)) {
      embed.setDescription(
        `\`❌\` | I need \`${command.userPrams.join(', ')}\` permission use this command.`,
      );
      return message.channel.send({ embeds: [embed] });
    }
